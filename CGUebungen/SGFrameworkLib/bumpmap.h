...
public:
    ...
    BumpMap(int unit = 1) : Texture(unit){}
    BumpMap(const QString& pfad, int unit = 1) : Texture(pfad, unit){mTextureUnit = unit;}

    //! \see DrawableProperty::turnOn()
    void turnOn(Shader* shader) override;

    //! \see DrawableProperty::turnOff()
